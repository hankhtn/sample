...
Namespace DevExpress.DevAV.ViewModels
    Partial Public Class EmployeeTaskCollectionViewModel
        ...
        Public Overridable Sub MarkComplete()
            Dim selectedIDs = SelectedEntities.Select(Function(x) x.Id).ToList()
            selectedIDs.ForEach(Sub(id)
                Dim entity = Entities.Single(Function(x) x.Id = id)
                If entity.Status <> EmployeeTaskStatus.Completed Then
                    entity.Status = EmployeeTaskStatus.Completed
                    entity.Completion = 100
                    Save(entity)
                    If entity Is SelectedEntity Then
