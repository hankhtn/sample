    ...
    Partial Public Class QuoteCollectionViewModel
        ...
        Public Function UpdateMapItems(ByVal start As Date, ByVal [end] As Date) As IEnumerable(Of Object)
            ...
            If items.Count() > 0 Then
                ...
                mapItems = items.Select(Function(group) New With {Key .Address = group.Key, Key .Total = group.CustomSum(Function(item) item.Value), Key .AbsSize = CDbl((group.CustomSum(Function(item) item.Value) - minValue) / dif)})
            Else
            ...
            Return mapItems
        End Function
        Private Function GetOpportunities(ByVal stage As Stage, Optional ByVal filterExpression As Expression(Of Func(Of Quote, Boolean)) = Nothing) As IList(Of QuoteMapItem)
            Dim unitOfWork = UnitOfWorkFactory.CreateUnitOfWork()
            Dim quotes = unitOfWork.Quotes.GetFilteredEntities(filterExpression).ActualQuotes()
