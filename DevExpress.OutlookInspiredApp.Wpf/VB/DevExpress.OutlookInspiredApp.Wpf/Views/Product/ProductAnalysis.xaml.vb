...
Namespace DevExpress.DevAV.Views.Product
    Partial Public Class ProductAnalysis
        ...
        Private Sub OnLoaded(ByVal sender As Object, ByVal e As RoutedEventArgs)
            DirectCast(splashScreenService, Mvvm.ISplashScreenService).ShowSplashScreen(String.Empty)
            LoadTemplate()
        End Sub
        ...
        Private Sub OnDocumentLoaded(ByVal sender As Object, ByVal e As EventArgs)
            LoadData()
            DirectCast(splashScreenService, Mvvm.ISplashScreenService).HideSplashScreen()
        End Sub
        Private Sub LoadData()
            ...
            Dim startReportsDate = financialReportItems.Min(Function(x) x.Date)
            For Each item As ProductsAnalysis.Item In financialReportItems
                ...
                Dim columnOffset As Integer = CInt(AnalysisPeriod.GetMonthOffsetFromStart(item.Date, startReportsDate) \ 12)
            ...
            For Each item As ProductsAnalysis.Item In financialDataItems
                Dim rowOffset As Integer = AnalysisPeriod.GetMonthOffsetFromStart(item.Date, startReportsDate) - 1
