    ...
    Public Class DashboardViewModel
        ...
        Public Property State() As String
            ...
            Set(ByVal value As String)
                ...
                OnStateChanged()
                Me.RaisePropertyChanged(Function(x) MapTitle)
            End Set
        End Property

        Private selectedState_Renamed As MapPath
        Public Property SelectedState() As MapPath
            Get
                Return selectedState_Renamed
            End Get
            Set(ByVal value As MapPath)
                selectedState_Renamed = value
        ...
        Private Sub OnStateChanged()
            CurrentMonthlySales = MonthlySales.Where(Function(x) x.State = dataState)
            CurrentTotalSales = TotalSales.Where(Function(x) x.State = dataState)
        End Sub
        Private Function GetStateByIndex() As String
            If states.Contains(state_Renamed) Then
                Return state_Renamed
            End If
            If stateIndex >= states.Count Then
                stateIndex = 0
            End If
            Dim tempVar As Integer = stateIndex
            stateIndex += 1
            Return states(tempVar)
        End Function
    End Class
