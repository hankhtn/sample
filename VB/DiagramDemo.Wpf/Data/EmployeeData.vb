...
Namespace DevExpress.Diagram.Demos
    ...
    Public NotInheritable Class EmployeesData
        ...
        Private Sub New()
        End Sub
        Public Shared ReadOnly FilteredEmployees As EmployeesWithPhotoData

        Shared Sub New()
            Using stream = DiagramDemoFileHelper.GetDataStream("FilteredEmployeesWithPhoto.xml")
                Dim serializer = New XmlSerializer(GetType(EmployeesWithPhotoData))
                FilteredEmployees = DirectCast(serializer.Deserialize(stream), EmployeesWithPhotoData)
            End Using
        End Sub
        Public Shared Function GetOrgChartEmployees() As IEnumerable(Of Object)
            Dim allEmployees As EmployeesWithPhotoData
            Using stream = DiagramDemoFileHelper.GetDataStream("EmployeesWithPhoto.xml")
                Dim serializer = New XmlSerializer(GetType(EmployeesWithPhotoData))
                allEmployees = DirectCast(serializer.Deserialize(stream), EmployeesWithPhotoData)
            End Using
        ...
        End Function
        #Region "id map"
        Private Shared idMap As New Dictionary(Of Integer, Integer())() From {
            {
                109, { 42, 117, 102 }
            },
            {
