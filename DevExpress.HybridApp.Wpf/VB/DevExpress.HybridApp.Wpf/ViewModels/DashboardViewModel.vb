...
Namespace DevExpress.DevAV.ViewModels
    Partial Public Class DashboardViewModel
        Private unitOfWork As IDevAVDbUnitOfWork = UnitOfWorkSource.GetUnitOfWorkFactory().CreateUnitOfWork()
        ...
        Private Sub SelectorReset(ByVal collection As ObservableCollection(Of Boolean), ByVal e As NotifyCollectionChangedEventArgs)
            If CBool(e.NewItems(0)) Then
                For i As Integer = 0 To collection.Count - 1
                    If i <> e.NewStartingIndex AndAlso collection(i) Then
                        collection(i) = False
                    End If
                Next i
            Else
                If Not collection.Contains(True) Then
