...
Namespace DevExpress.DevAV.ViewModels
    Public NotInheritable Class AnalysisPeriod
        ...
        Public Shared Function GetMonthOffsetFromStart(ByVal dateTime As Date, ByVal startDateTime As Date) As Integer
    ...
    Public Module ProductsAnalysis
        ...
        Public Function GetFinancialReport(ByVal UnitOfWork As IDevAVDbUnitOfWork) As IEnumerable(Of Item)
            ...
            Dim ordersStartDate = orders.Min(Function(x) x.OrderDate)
            Dim orderItems = From oi In UnitOfWork.OrderItems
                             Join o In orders On oi.OrderId Equals o.Id
                             Select New With {Key .Product = oi.Product, Key .Total = oi.Total, Key .FY = ((o.OrderDate.Year - ordersStartDate.Year) * 12 + (o.OrderDate.Month - ordersStartDate.Month)) / 12}
            Return From oi In orderItems
                   Group oi By GroupKey = New With {Key oi.Product, Key oi.FY} Into g = Group
                   Select New Item With {.ProductName = GroupKey.Product.Name, .Year = ordersStartDate.Year + CInt((GroupKey.FY)), .Month = ordersStartDate.Month, .Total = If(g.Select(Function(x) CType(x.Total, Decimal?)).Sum(), 0)}
        End Function
        <System.Runtime.CompilerServices.Extension> _
    ...
    Public Module CustomersAnalysis
        ...
        Public Function GetSalesReport(ByVal UnitOfWork As IDevAVDbUnitOfWork) As IEnumerable(Of Item)
            ...
            Dim orderItems = From oi In UnitOfWork.OrderItems
                             Join o In orders On oi.OrderId Equals o.Id
                             Select New With {Key .Customer = o.Customer, Key .Total = oi.Total, Key .FY = ((o.OrderDate.Year - ordersStartDate.Year) * 12 + (o.OrderDate.Month - ordersStartDate.Month)) / 12}
            Return From oi In orderItems
                   Group oi By GroupKey = New With {Key oi.Customer, Key oi.FY} Into g = Group
                   Select New Item With {.CustomerName = GroupKey.Customer.Name, .Year = ordersStartDate.Year + CInt((GroupKey.FY)), .Month = ordersStartDate.Month, .Total = If(g.Select(Function(o) CType(o.Total, Decimal?)).Sum(), 0)}
        End Function
        <System.Runtime.CompilerServices.Extension> _
