    ...
    Public NotInheritable Class TeamData
        ...
        Private Shared Function CreateCalendars() As IEnumerable(Of TeamCalendar)
            Return New TeamCalendar() {
                New TeamCalendar() With {.Id = 0, .Name = "My Calendar"},
                New TeamCalendar() With {.Id = 1, .Name = "Team Calendar"}
            }
        End Function
        ...
        Private Shared Function CreateBirthdayAppt(ByVal employee As Employee) As TeamAppointment
            ...
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Pattern)), .AllDay = True, .Start = [date], .End = [date].AddDays(1), .Subject = String.Format("{0}'s Birthday", employee.FirstName), .Status = 0, .Label = 8, .CalendarId = 0}
            appt.RecurrenceInfo = New RecurrenceInfo() With {.AllDay = True, .Start = [date], .Month = [date].Month, .DayNumber = [date].Day, .WeekOfMonth = WeekOfMonth.None, .Type = RecurrenceType.Yearly, .Range = RecurrenceRange.NoEndDate}.ToXml()
            Return appt
        End Function
        ...
        Private Shared Function CreateConferenceAppts(ByVal start As Date) As IEnumerable(Of TeamAppointment)
            ...
            Dim thisWeekList() As Tuple(Of String, DateTime) = { Tuple.Create("DevExpress MVVM Framework", newStart.AddDays(1).AddHours(15)), Tuple.Create("New Theme Designer", newStart.AddDays(2).AddHours(14)), Tuple.Create("GridControl Performance Optimization", newStart.AddDays(3).AddHours(16)), Tuple.Create("WinForms and DirectX", newStart.AddDays(4).AddHours(16))}

            newStart = start.AddDays(-7)
            Dim prevWeekList() As Tuple(Of String, DateTime) = { Tuple.Create("LOB applications", newStart.AddDays(1).AddHours(13)), Tuple.Create("Module Injection Framework", newStart.AddDays(2).AddHours(16)), Tuple.Create("Git tricks", newStart.AddDays(3).AddHours(10)), Tuple.Create("Machine learning", newStart.AddDays(4).AddHours(11))}

            newStart = start.AddDays(7)
            Dim nextWeekList() As Tuple(Of String, DateTime) = { Tuple.Create("Azure", newStart.AddDays(1).AddHours(13)), Tuple.Create("WCF Services", newStart.AddDays(2).AddHours(16)), Tuple.Create("Docking Floating Panels", newStart.AddDays(3).AddHours(10)), Tuple.Create("Personal Time Management", newStart.AddDays(4).AddHours(11))}

            newStart = start.AddDays(14)
            Dim nextNextWeekList() As Tuple(Of String, DateTime) = { Tuple.Create("Entity Framework Core", newStart.AddDays(1).AddHours(10)), Tuple.Create(".Net Core", newStart.AddDays(2).AddHours(16))}

            Dim list As IEnumerable(Of Tuple(Of String, DateTime)) = thisWeekList.Concat(prevWeekList).Concat(nextWeekList).Concat(nextNextWeekList)

            Dim commonList As New List(Of Tuple(Of String, DateTime))()
        ...
        Private Shared Function CreateConferenceAppt(ByVal subject As String, ByVal start As Date) As TeamAppointment
            ...
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = False, .Start = start, .End = start.AddHours(1.5), .Subject = String.Format("Conference: {0}", subject), .Description = String.Format("{0} {1} tells us about {2}.", emp.FirstName, emp.LastName, subject), .Status = 2, .Label = 2, .Location = "Conference Room", .CalendarId = 1}
            Return appt
        End Function
        ...
        Private Shared Function CreateMeetingRecurrenceAppt(ByVal subject As String, ByVal start As Date) As TeamAppointment
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Pattern)), .AllDay = False, .Start = start, .End = start.AddHours(1), .Subject = subject, .Status = 2, .Label = 2, .CalendarId = 1}
            appt.RecurrenceInfo = New RecurrenceInfo() With {.Start = start, .Type = RecurrenceType.Weekly, .WeekDays = WeekDays.Friday, .Month = 12, .Range = RecurrenceRange.NoEndDate}.ToXml()
            Return appt
        End Function
        Private Shared Function CreateLunchAppt(ByVal emp As Employee, ByVal start As Date) As TeamAppointment
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = False, .Start = start, .End = start.AddHours(1), .Subject = String.Format("Lunch with {0}", emp.FirstName), .Status = 3, .Label = 3, .CalendarId = 0}
            Return appt
        End Function
        ...
        Private Shared Function CreatePhoneCallAppt(ByVal emp As Employee, ByVal start As Date, Optional ByVal duration? As TimeSpan = Nothing) As TeamAppointment
            ...
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = False, .Start = newStart, .End = newEnd, .Subject = String.Format("Phone Call with {0}", emp.FirstName), .Status = 2, .Label = 10, .CalendarId = 0}
            Return appt
        End Function

        Private Shared Function CreateVacationsAppts(ByVal start As Date) As IEnumerable(Of TeamAppointment)
            Return {
                New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = True, .Start = start.AddMonths(-6), .End = start.AddMonths(-6).AddDays(14), .Subject = String.Format("Vacation"), .Status = 0, .Label = 4, .CalendarId = 0},
                New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = True, .Start = start.AddMonths(6), .End = start.AddMonths(6).AddDays(14), .Subject = String.Format("Vacation"), .Status = 0, .Label = 4, .CalendarId = 0},
                New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = True, .Start = start.AddDays(4), .End = start.AddDays(8), .Subject = String.Format("Vacation"), .Status = 0, .Label = 4, .CalendarId = 0}
            }
        End Function
        ...
        Private Shared Function CreateCarWashAppt(ByVal start As Date) As TeamAppointment
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = False, .Start = start, .End = start.AddHours(1), .Subject = String.Format("Car Wash"), .Status = 3, .Label = 3, .CalendarId = 0}
            Return appt
        End Function
        ...
        Private Shared Function CreateCompanyBirthdayAppts(ByVal start As Date) As IEnumerable(Of TeamAppointment)
            ...
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Pattern)), .AllDay = True, .Start = newStart, .End = newStart.AddDays(1), .Subject = "Company Birthday Party", .Status = 0, .Label = 8, .CalendarId = 1}
            appt.RecurrenceInfo = New RecurrenceInfo() With {.AllDay = True, .Start = newStart, .Type = RecurrenceType.Yearly, .Month = newStart.Month, .DayNumber = newStart.Day, .WeekOfMonth = WeekOfMonth.None, .Range = RecurrenceRange.NoEndDate}.ToXml()
        ...
        Private Shared Function CreateTrainingAppts(ByVal start As Date) As IEnumerable(Of TeamAppointment)
            ...
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Pattern)), .AllDay = False, .Start = newStart, .End = newStart.AddHours(1.5), .Subject = "Sport Training", .Status = 1, .Label = 3, .CalendarId = 0}
            appt.RecurrenceInfo = New RecurrenceInfo() With {.AllDay = False, .Start = newStart, .Type = RecurrenceType.Weekly, .WeekDays = WeekDays.Monday Or WeekDays.Wednesday Or WeekDays.Friday, .Range = RecurrenceRange.NoEndDate}.ToXml()
            Return { appt }
        End Function
        ...
        Private Shared Function CreatePayBillsAppts(ByVal start As Date) As IEnumerable(Of TeamAppointment)
            ...
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Pattern)), .AllDay = True, .Start = newStart, .End = newStart.AddDays(1), .Subject = "Pay Bills", .Status = 0, .Label = 3, .CalendarId = 0}
            appt.RecurrenceInfo = New RecurrenceInfo() With {.AllDay = True, .Start = newStart, .Type = RecurrenceType.Monthly, .DayNumber = newStart.Day, .WeekOfMonth = WeekOfMonth.None, .Range = RecurrenceRange.NoEndDate}.ToXml()
            Return { appt }
        End Function
        ...
        Private Shared Function CreateDentistAppt(ByVal start As Date) As TeamAppointment
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = False, .Start = start, .End = start.AddHours(2), .Subject = String.Format("Dentist"), .Status = 3, .Label = 3, .CalendarId = 0}
            Return appt
        End Function
        ...
        Private Shared Function CreateDinnerAppt(ByVal start As Date, Optional ByVal duration? As TimeSpan = Nothing) As TeamAppointment
            ...
            Dim appt = New TeamAppointment() With {.AppointmentType = CInt((AppointmentType.Normal)), .AllDay = False, .Start = newStart, .End = newEnd, .Subject = String.Format("Dinner"), .Status = 0, .Label = 5, .CalendarId = 0}
            Return appt
        End Function
    End Class
End Namespace
