...
Namespace RichEditDemo
    Partial Public Class CustomDraw
        ...
        Private _searchResult As New List(Of FixedRange)()
        ...
        Private Property CurrentItemIndex() As Integer
            ...
            Set(ByVal value As Integer)
                If value < 0 OrElse value >= SearchResult.Count Then
                    Return
                End If
        ...
        Private Sub RichEditControl_Loaded(ByVal sender As Object, ByVal e As RoutedEventArgs)
            richEdit.ReplaceService(Of IRichEditCommandFactoryService)(New CustomsRichEditCommandFactoryService(richEdit, richEdit.GetService(Of IRichEditCommandFactoryService)(), Me.searchTextBox))
            FindText()
        End Sub
        Private Sub RichEditControl_BeforePagePaint(ByVal sender As Object, ByVal e As BeforePagePaintEventArgs)
            If e.CanvasOwnerType = CanvasOwnerType.Printer OrElse SearchResult.Count = 0 Then
                Return
            End If
        ...
        Private Sub UpdateSearchOptionsUI()
            Me.navigationButtonEdit.Visibility = If((Not String.IsNullOrEmpty(Me.searchTextBox.Text)) AndAlso SearchResult.Count > 0, Visibility.Visible, Visibility.Hidden)
            For Each button As ButtonInfo In Me.searchTextBox.Buttons
                button.Visibility = If((Not String.IsNullOrEmpty(Me.searchTextBox.Text)), Visibility.Visible, Visibility.Hidden)
            Next button
            If String.IsNullOrEmpty(Me.searchTextBox.Text) Then
                Me.labelResultCount.Text = String.Empty
            ElseIf SearchResult.Count = 0 Then
                Me.labelResultCount.Text = "No matches"
            Else
                Me.labelResultCount.Text = String.Format("{0} of {1} matches", CurrentItemIndex + 1, SearchResult.Count)
            End If
        End Sub
        Private Sub FindText()
            SearchResult.Clear()
            Dim textToSearch As String = Me.searchTextBox.Text
        ...
        Private Function GetSearchOptions() As SearchOptions
            ...
            If Me.edtMatchCase.IsChecked.Value Then
                result = result Or SearchOptions.CaseSensitive
            End If
            If Me.edtFindWholeWordsOnly.IsChecked.Value Then
                result = result Or SearchOptions.WholeWord
            End If
            Return result
        End Function
