...
Namespace DiagramDemo
    Public Class DiagramEventsInfo
        Private Shared ReadOnly EAction As New EventArgInfo(Of ItemsActionKind)("Action", EnumArgValues( { ItemsActionKind.Move, ItemsActionKind.MoveCopy, ItemsActionKind.Resize, ItemsActionKind.Rotate, ItemsActionKind.Copy, ItemsActionKind.Delete }, Function(x) False))
        Private Shared ReadOnly EStage As New EventArgInfo(Of DiagramActionStage)("Stage", EnumArgValues( { DiagramActionStage.Start, DiagramActionStage.Continue, DiagramActionStage.Finished, DiagramActionStage.Canceled }, Function(x) x <> DiagramActionStage.Continue))
        Private Shared ReadOnly EConnectorPointType As New EventArgInfo(Of ConnectorPointType)("ConnectorPointType", EnumArgValues( { ConnectorPointType.Begin, ConnectorPointType.End }, Function(x) x = ConnectorPointType.End))
        Private Shared ReadOnly EConnector As New EventArgInfo(Of DiagramConnector)("Connector", {
            New EventArgValueInfo(Of DiagramConnector)(Function(x) "e.Connector.GetDiagram() is null", Nothing, Function(x) x.GetDiagram() Is Nothing),
            New EventArgValueInfo(Of DiagramConnector)(Function(x) "e.Connector.GetDiagram() is not null", True, Function(x) x.GetDiagram() IsNot Nothing)
        })
        Private Shared ReadOnly EItemUsage As New EventArgInfo(Of ItemUsage)("ItemUsage", EnumArgValues( { ItemUsage.Diagram, ItemUsage.ToolboxPreview }, Function(x) x = ItemUsage.Diagram))
        ...
        End Class
        Private Function Initialize(Of TEventArgs As EventArgs)(ByVal title As String, ByVal isChecked As Boolean, ByVal format As String, ParamArray ByVal formatArgs() As Func(Of TEventArgs, Object)) As DiagramEventNode
            ...
            eventsOwner.GetType().GetEvent(title).AddEventHandler(eventsOwner, New EventHandler(Of TEventArgs)(Sub(sender, e)
                If node.ActualIsChecked Then
                    addToLog(title, format, formatArgs.Select(Function(x) x(e)).ToArray())
                End If
            End Sub))
            Return node
        End Function
        Private Function Initialize(Of TEventArgs As EventArgs, TArg1)(ByVal title As String, ByVal isChecked? As Boolean, ByVal arg1 As EventArgInfo(Of TArg1), ByVal format As String, ParamArray ByVal formatArgs() As Func(Of TEventArgs, Object)) As DiagramEventNode
            Dim node = DiagramEventNode.Create(title, isChecked, DiagramEventNodeKind.EventNode)
            ...
            eventsOwner.GetType().GetEvent(title).AddEventHandler(eventsOwner, New EventHandler(Of TEventArgs)(Sub(sender, e)
                ...
                If children.First(Function(x) x.Item1(v1)).Item2.ActualIsChecked Then
                    addToLog(title, format, formatArgs.Select(Function(x) x(e)).ToArray())
                End If
            End Sub))
            Return node
        End Function
        Private Function Initialize(Of TEventArgs As EventArgs, TArg1, TArg2)(ByVal title As String, ByVal isChecked? As Boolean, ByVal arg1 As EventArgInfo(Of TArg1), ByVal arg2 As EventArgInfo(Of TArg2), ByVal format As String, ParamArray ByVal formatArgs() As Func(Of TEventArgs, Object)) As DiagramEventNode
            Dim node = DiagramEventNode.Create(title, isChecked, DiagramEventNodeKind.EventNode)
            ...
            eventsOwner.GetType().GetEvent(title).AddEventHandler(eventsOwner, New EventHandler(Of TEventArgs)(Sub(sender, e)
                Dim v1 = CType(e.GetType().GetProperty(arg1.Title).GetValue(e, Nothing), TArg1)
                Dim v2 = CType(e.GetType().GetProperty(arg2.Title).GetValue(e, Nothing), TArg2)
                If children.First(Function(x) x.Item1(v1, v2)).Item2.ActualIsChecked Then
                    addToLog(title, format, formatArgs.Select(Function(x) x(e)).ToArray())
                End If
            End Sub))
            Return node
