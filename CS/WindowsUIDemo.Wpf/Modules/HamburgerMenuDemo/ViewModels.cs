...
#region MainViewModel
    ...
    public class HamburgerMenuDemoViewModel {
        ...
        IList<BottomBarItemViewModelBase> CreateBottomBarItems() {
            ...
            var items = new List<BottomBarItemViewModelBase>();
        ...
        void UpdateMailFilter(MailType mailType) {
            ...
            if (Data != null && Data.Count() > 0)
                FocusedRow = Data.Where(p => p.MailType == mailType).First();
        }
        int GetMailCountByType(MailType type) {
            return Data.Where(p => p.MailType == type).Count();
        }
        IList<HamburgerItemViewModelBase> CreateMenuItems() {
            ...
            var subItems = new List<HamburgerSubItemViewModel>() {
                new HamburgerSubItemViewModel("Inbox", GetMailCountByType(MailType.Inbox), null, true, this) { Command = command, CommandParameter = MailType.Inbox },
