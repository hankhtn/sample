...
Namespace SchedulingDemo
    Public Class GymData
        ...
        Private Shared ReadOnly kindOfTraining() As Training = {
            New Training() With {.Id = TrainingKind.Bodybuilding, .Name = "Bodybuilding", .Color = Color.FromRgb(&HC1, &HF4, &H9C)},
            New Training() With {.Id = TrainingKind.Yoga, .Name = "YOGA", .Color = Color.FromRgb(&HFF, &HC2, &HBE), .Description = "Yoga is a group of physical, mental, and spiritual practices or disciplines."},
            New Training() With {.Id = TrainingKind.Zumba, .Name = "Zumba", .Color = Color.FromRgb(&HA8, &HD5, &HFF), .Description = "Zumba involves dance and aerobic movements performed to energetic music."},
            New Training() With {.Id = TrainingKind.Boxing, .Name = "Boxing", .Color = Color.FromRgb(&H8D, &HE9, &HDF), .Description = "Boxing is a combat sport in which two people, wearing protective gloves, throw punches at each other for a predetermined set of time in a boxing ring."},
            New Training() With {.Id = TrainingKind.Cxworx, .Name = "CXWORX", .Color = Color.FromRgb(&HF3, &HE4, &HC7), .Description = "Exercising muscles around the core."},
            New Training() With {.Id = TrainingKind.BodyPump, .Name = "BodyPump", .Color = Color.FromRgb(&HF4, &HCE, &H93), .Description = "BodyPump is a barbell workout for anyone looking to get lean, toned and fit – fast."},
            New Training() With {.Id = TrainingKind.Pilates, .Name = "Pilates", .Color = Color.FromRgb(&HC7, &HF4, &HFF), .Description = "Pilates classes build strength, flexibility and lean muscle tone with an emphasis on lengthening the body and aligning the spine."},
            New Training() With {.Id = TrainingKind.Abl, .Name = "ABL", .Color = Color.FromRgb(&HCF, &HDB, &H98), .Description = "ABL program is specially designed for women. Strength training aims to develop the muscles of the abdomen, buttocks and legs."},
            New Training() With {.Id = TrainingKind.Tabata, .Name = "Tabata", .Color = Color.FromRgb(&HE0, &HCF, &HE9), .Description = "Tabata is a form of high intensity interval training designed to get your heart rate up in that very hard anaerobic zone for short periods of time."},
            New Training() With {.Id = TrainingKind.PrivateTraining, .Name = "Private Training", .Color = Color.FromRgb(&HFF, &HF7, &HA5)}
        }
        Private Shared ReadOnly trainerNames() As Trainer = {
            New Trainer() With {.Id = TrainingKind.Bodybuilding, .Name = "Mark Oliver"},
            New Trainer() With {.Id = TrainingKind.Yoga, .Name = "Greta Sims"},
            New Trainer() With {.Id = TrainingKind.Zumba, .Name = "Cindy Stanwick"},
            New Trainer() With {.Id = TrainingKind.Boxing, .Name = "Robert Reagan"},
            New Trainer() With {.Id = TrainingKind.Cxworx, .Name = "Andrew Fuller"},
            New Trainer() With {.Id = TrainingKind.BodyPump, .Name = "Sammy Hill"},
            New Trainer() With {.Id = TrainingKind.Pilates, .Name = "Laura Callahan"},
            New Trainer() With {.Id = TrainingKind.Abl, .Name = "Terry Bradley"},
            New Trainer() With {.Id = TrainingKind.Tabata, .Name = "Sandy Bright"},
            New Trainer() With {.Id = TrainingKind.PrivateTraining, .Name = "Lucas Smith"}
        }
        ...
        Private Shared Function GetBodybuildingEvent(ByVal id As Integer, ByVal [date] As Date, ByVal dayCount As Integer) As GymEvent
            Dim firstDate As New Date([date].Year, [date].Month, [date].Day)
        ...
        Private Shared Function GetEvents(ByVal id As Integer, ByVal [date] As Date, ByVal dayCount As Integer) As ObservableCollection(Of GymEvent)
            ...
            Do While dateTime < lastDate
                ...
                Do While startTime < endDate
                    ...
                    id += 1
                    trainingId += 1
                    If trainingId >= kindOfTraining.Length Then
                        trainingId = 1
                    End If
                    startTime = startTime.Add(New TimeSpan(rnd.Next(1, 4), 0, 0))
                Loop
        ...
        Private privateTrainers As ObservableCollection(Of Trainer)
        Public Property Trainers() As ObservableCollection(Of Trainer)
            Get
                Return privateTrainers
            End Get
            Private Set(ByVal value As ObservableCollection(Of Trainer))
                privateTrainers = value
            End Set
        End Property
        Private privateTrainings As ObservableCollection(Of Training)
        Public Property Trainings() As ObservableCollection(Of Training)
            ...
            Private Set(ByVal value As ObservableCollection(Of Training))
                privateTrainings = value
            End Set
        End Property
        Private privateGymEvents As ObservableCollection(Of GymEvent)
        Public Property GymEvents() As ObservableCollection(Of GymEvent)
            Get
                Return privateGymEvents
            End Get
            Private Set(ByVal value As ObservableCollection(Of GymEvent))
                privateGymEvents = value
            End Set
        End Property
    End Class
