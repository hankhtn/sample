...
Namespace DevExpress.DevAV
    Public Class InvoiceEditBehavior
        ...
        Private Sub InitSpreadsheet()
            If AssociatedObject Is Nothing OrElse (Not AssociatedObject.IsLoaded) Then
                Return
            End If
            LoadInvoiceTemplate()
            ParseDataSource()
        ...
        Private Sub OnPreviewMouseLeftButtonDown(ByVal sender As Object, ByVal e As System.Windows.Input.MouseButtonEventArgs)
        ...
        Private Sub CreateInvoiceHelper()
            Dim collections As New OrderCollections()
            Dim actions As New EditActions()

            If unitOfWork IsNot Nothing Then
                actions.ActivateEditor = Sub()
                    Dim activeSheet = AssociatedObject.ActiveWorksheet
                    If activeSheet.Name = CellsHelper.InvoiceWorksheetName AndAlso activeSheet.CustomCellInplaceEditors.GetCustomCellInplaceEditors(activeSheet.Selection).Any() Then
                        AssociatedObject.OpenCellEditor(XtraSpreadsheet.CellEditorMode.Edit)
                    End If
                End Sub
                actions.CloseEditor = Sub()
                    If AssociatedObject.IsCellEditorActive Then
        ...
        Private Sub OnCustomCellEdit(ByVal sender As Object, ByVal e As SpreadsheetCustomCellEditEventArgs)
            If (Not e.ValueObject.IsText) OrElse unitOfWork Is Nothing Then
                Return
            End If
            Dim editorInfo = CellsHelper.FindEditor(e.ValueObject.TextValue)
            If editorInfo IsNot Nothing Then
