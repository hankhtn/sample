...
Namespace DiagramDemo
    ...
    Partial Public Class TournamentModule
        Inherits DiagramDemoModule

        Public Sub New()
            InitializeComponent()
            diagramControl.Controller.RegisterRoutingStrategy(ConnectorType.RightAngle, New RightAngleRoutingStrategy() With {.ItemMargin = CInt((Me.diagramControl.TreeLayoutHorizontalSpacing)) \ 2})
            DataContext = New TournamentViewModel()
        End Sub
        Private Sub diagramControl_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Input.MouseButtonEventArgs)
            Dim item = Me.diagramControl.CalcHitItem(Function(el) e.GetPosition(el))
            ...
            If clickedItem IsNot Nothing AndAlso Not(TypeOf clickedItem Is DiagramContainer) Then
                ...
                If commandContainer IsNot Nothing Then
                    ...
                    Me.diagramControl.SelectItem(resultShape)
                    Me.diagramControl.Commands.Execute(DiagramCommandsBase.EditCommand)
                End If
            End If
        End Sub
